...
namespace TextRPG_TeamProject2nd.Manager
{
    ...
    internal class FileManager
    {
        ...
        public bool LoadPlayer()
        {
            ...
            player.GetInfo().consumeId = int.Parse(inData[3]);
            player.GetInfo().level = int.Parse(inData[4]);
            player.GetInfo().hp = int.Parse(inData[5]);
            player.GetInfo().maxHp = int.Parse(inData[6]);
            player.GetInfo().attack = int.Parse(inData[7]);
            player.GetInfo().defence = int.Parse(inData[8]);
            player.GetInfo().actionPoint = int.Parse(inData[9]);
            player.GetInfo().exp = int.Parse(inData[10]);
            player.GetInfo().maxExp = int.Parse(inData[11]);
            player.GetInfo().money = int.Parse(inData[12]);
            player.GetInfo().race = inData[13];
            player.GetInfo().questId = int.Parse(inData[14]);
            player.GetInfo().questProgress = int.Parse(inData[15]);
